...
условия выполняются, участок признаётся оптимальным. При невыполнении одного условия - подходящим, при невыполнении двух
- ограниченно пригодным. В остальных случаях участок считается
непригодным.
...
Откроется веб-интерфейс по адресу `http://localhost:8502`.

По умолчанию приложение ожидает полные файлы
`data/processed/bonitet_yar.csv` и `data/processed/suitability_yar.csv`.
Если они отсутствуют, система автоматически переключается на
демонстрационные выборки из `data/sample/`, содержащие по 1000
наблюдений. Этого достаточно для проверки работоспособности интерфейса.
